...
void process2(int input_fd, int output_fd) {
    ...
    snprintf(int_buffer, sizeof(int_buffer), "%d", count);
    int_buffer[sizeof(int)] = '\0';
    size_t write_size = write(output_fd, int_buffer, sizeof(int_buffer));
    if (write_size != sizeof(int_buffer)) {
...
void process3(int input_fd, char output_path[]) {
    ...
    ssize_t read_size = read(input_fd, buffer, sizeof(int));
    if (read_size < 0) {
    ...
    size_t write_size = write(output_fd, buffer, sizeof(buffer));
    if (write_size != sizeof(int) + 2) {
